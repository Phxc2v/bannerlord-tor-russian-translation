...
**Крупная чистка транслита, канона, структурных багов и ink-квестов — ~300 правок во всех 34 файлах.**

**Тиры рыцарства Бретоннии (тултип боевого духа отряда):**

Все 7 тиров были транслитом, из-за чего в тултипе «Боевой дух отряда» отображалась мусорная строка `Уннигтли −74`. Исправлено:
- `Уннигтли` → **«Бесчестный»**
...
- Уровень Ваагха `Унновн` → **«Неизвестно»**.

**Диалоги чарователей всех фракций:**

14 строк `TODO: БРЕТОННИЯ - ...` / `TODO: АпРАЙ — ...` / `TODO: ИМПЕРИЯ - ...` заменены на осмысленные реплики для Бретоннии, Эонира, Асрай и Империи. Приветствия посетителей, коллег по ремеслу, первые визиты — теперь везде полноценный текст.

**Ink-квесты больше не сыпят английским:**

Раньше `Traveling Merchant`, `Cabin in the Woods Locked`, `Duel`, `Meadow`, `Fair`, `Minstrel` и другие тексты показывались по-английски, если игрок не запустил `УСТАНОВИТЬ_ink.bat`. Теперь:

- **123 новые XML-строки** для `#STR_`-тегов всех 27 ink-квестов — перевод подтягивается автоматически без ручной установки.
- `УСТАНОВИТЬ_ink.bat` **полностью переписан** — раньше мог закрываться мгновенно с ошибкой `'574678)' is not recognized` / `'Blade' is not recognized`. Причины: LF вместо CRLF, русский текст в echo, `&` в `Mount & Blade`, скобки `(x86)` в `%~1` внутри `if (…)` блоков. Теперь ASCII, CRLF, `call :try` вместо `for`, `!VAR!` delayed expansion везде где надо. Работает из Program Files без требования админ-прав.
- Запуск bat нужен только для 12 квестов без `#STR_` тегов («Да Босс Пробуждается», орочьи и нурглистские видения) — для остальных RU подтягивается из XML.

**Имперские дезертиры — инверсия смысла:**

В EN-оригинале `Deserter / Deserter Sergeant / Deserter Scout` (бандиты-дезертиры), в переводе было `Добровольный Ополченец` — противоположный смысл. Исправлено:
- → **«Дезертир»**, **«Сержант-дезертир»**, **«Разведчик-дезертир»**
- `Империя Десертер Клан` → **«Клан Имперских Дезертиров»**
- `Имперская Государственная Войска` → **«Имперский государственный солдат»** (`Empire State Troop` — это один пехотинец, не армия)

**Голый английский в RU-строках:**

- `Cairn` → **Каирн** (призрачные доспехи ×2)
...
- `helpnecromancersuccess/fail` — в тексте был id предмета `tor_vc_weapon_staff_nm_001` вместо перевода; восстановлены нормальные фразы.

**Транслит-мусор в названиях и описаниях:**

- `Лигтнинг` → **Молния** (×4 — урон заклинаний, описания карьер, печать Ульрика, печать Мананна)
...
Ранее из-за визуально похожих букв в ряде КАПС-фраз заглавные «С» и «З» были заменены строчными. Восстановлено:
- `МЯпО` → **МЯСО** (приветствие тролля)
...
- `анаюсь` → **Владею**, `анаете` → **Знаете** (предыстория + ответы странников)
- `ПОДпТУПОВ` → **ПОДЛЮГ**, `пюда` → **сюда**, `ДИп` → **ДИС** (Дисциплина)
...
**Имена странников-компаньонов (wanderer NPC):**

- `Тейн` → **Тан** (dwarf `the Thane`)
- `Коваль Рун` → **Рунекузнец** (`the Runesmith` — отсюда Тектон-кузнец больше не «Коваль Рун»)
- `Дамсела Леди` → **Дева Владычицы** (Bretonnian Damsel of the Lady)
- `Машан` → **Маршан** (Bretonnian Marchand — купец)
...
~40 перков разных карьер (рыцари, маги, орки-боссы, вампиры, жрецы) использовали `+X жизней` / `жизненных очков` / `жизненным силам` — неверный перевод `health`. Везде теперь корректно:
...
Плюс школы Имперской Магии:
- `Арканного канала / Арканного Кондуита` → **канала Тайной магии**
- `Арканная канализация / Канализация Арканного Кондуита` → **Поток тайной магии**
- `Арканные/Арканские свитки` → **магические свитки**
- Имя тома `Де Арканис Кадон` оставлено как имя собственное.

И:
- `Однажды в день отправляйтесь на охоту.` → **Раз в день отправляйтесь на охоту.** (калька с `Once per day`)
- **Китайские иероглифы `单手`** в RU-строке перка «Хранитель Торговани» → **«одноручного»**.
- `войск «Меньшие нежитьи»` → **«Меньшая нежить»** / **«Меньшей нежити»** (8 строк, Де Арканис / Книга Всорана / Кодекс Мортифика / Либер Мортис)
- `+1 жизнь` → **+1 здоровья**, опечатка `аа каждый` → **За каждый** (перк Некроманта)

**Крафт-компоненты гномьих топоров:**

- `Гном Благородный Топор Хеад/Хандле` → **Головка/Древко благородного гномьего топора** (×3)

**Канон-единообразие (все 34 файла):**

- `Ател Лорен` → **Атель Лорен** (×18 — пропущена «ь»)
- `Талабекленд` → **Талабекланд** (×6)
- `Лайонесс` → **Лионесс** (×6)
- `Альдорф` → **Альтдорф**, `Каркасон` → **Каркассонн**, `Штирланд` → **Стирланд**
- `Улрик` → **Ульрик** (×3, включая `Ар-Улрик Эмил Валгеир` → **Ар-Ульрик Эмиль Вальгейр**)
...
Обнаружены и ликвидированы **88 дублирующихся ID** между файлами `str_tor_ror_settlement_templates-rus.xml` и `str_tor_settlements-rus.xml` с расходящимся текстом (двигло рандомно выбирало вариант). Для каждого выбран лучший перевод, попутно исправлены канон-ошибки:
...
- `вардансеров` → **Танцоров Войны** (Wardancers)
- `Смертных Джек` → **Смертоджеков** (Deathjacks)
- … и ещё 80. Дополнительно 5 одинаковых дублей вычищены, `Орка-Селянка` унифицирована.
...
**Грамматика род/число (для игрока-женщины):**

- `{HERO_NAME} стал Вампиром-Крестоносцем` → **становится Рыцарем-Вампиром Крови**
- `{HERO_NAME} стал Нексархом` → **становится Некрархом**
- Диалоги игрока с инженером Нульна и гномом-скитальцем перефразированы без маскулинных глаголов прошедшего времени — теперь работают и для героинь-женщин.

**Прочее:**

- `+40% лигтнинг Урон` → **+40% к урону молнии**
- `+20% Лигтнинг Урон` → **+20% к урону молнии** (печать Мананна)
- `СвежаяКровь` → **Свежая Кровь** (обращение хузаитского лорда)
- `плотлингов` → **снотлингов** (предыстория зеленокожего)

**Итого:** ~300 исправлений во всех 34 XML-файлах (16 028 строк) + полная переработка установочного bat-скрипта. Проект прошёл сплошную проверку XML-целостности — 0 структурных ошибок.
