...
//Scenarios notes
    ...
    //Future Options/Additions
        ...
        //Someone answers when you Постучать в дверь
        //Burn down the cabin
...
===Start===
    Ваша группа движется по лесу, когда вы обнаруживаете избушку. #STR_Start1
    
    *[Подойти к избушке]->Approach
    *[Продолжить путь (Уйти)]You decide it is better to move on for now.->END
    
===Approach===

As you Подойти к избушке you can see that it is heavily boarded up. The only door on the cabin seems to be locked tight. As you examine the door you see that the door is {DoorText} and that the lock on it is {LockText}. #STR_Approach1
->choice1  

    =choice1
    What will your party do?
    *[Постучать в дверь]You knock but no one answers.->Approach.choice1
    
    //Pick the lock (Roguery)
        *[Подобрать замок на двери {PartyRogueryCheckText}]
    ...
    //Blow up the door (Spellcraft)
        *{PartyCanCastSpell == true}[Снести дверь взрывом {PartySpellcraftCheckText}]
    ...
    //Break down the door (Vigor)
        *[Прорубить дверь {PartyVigorCheckText}]
...
Ваша партия проникает внутрь шалупы и обнаруживает, что здесь кто-то или что-то запасило припасы. #STR_Inside1
->choice2 

    =choice2
        *[Возьмите припасы ({RewardText})]
